...
def start_transfer_session(
    ...
    # Get cleaning interval and magnet from route mapping
    ...
    magnet_id = route_mapping.magnet_id if route_mapping else session_data.magnet_id
...
@app.put("/api/bins/{bin_id}", response_model=schemas.Bin)
def update_bin(bin_id: int, bin_data: schemas.BinUpdate, db: Session = Depends(get_db)):
    db_bin = db.query(models.Bin).filter(models.Bin.id == bin_id).first()
    if not db_bin:
        raise HTTPException(status_code=404, detail="Bin not found")
...
@app.post("/api/magnets", response_model=schemas.Magnet)
def create_magnet(magnet: schemas.MagnetCreate, db: Session = Depends(get_db)):
    db_magnet = models.Magnet(**magnet.dict())
    db.add(db_magnet)
    db.commit()
    db.refresh(db_magnet)
    return db_magnet
...
@app.put("/api/magnets/{magnet_id}", response_model=schemas.Magnet)
def update_magnet(magnet_id: int, magnet: schemas.MagnetUpdate, db: Session = Depends(get_db)):
    db_magnet = db.query(models.Magnet).filter(models.Magnet.id == magnet_id).first()
    if not db_magnet:
        raise HTTPException(status_code=404, detail="Magnet not found")
